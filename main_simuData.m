...
occ_grid = robotics.OccupancyGrid(bwimage,25); % 25 Pixel == 1 Meter
...
hold on
show(occ_grid);
title({'Occupancy Grid', 'Skalierung: 25 Pixel ≙ 1 Meter'})
xlabel('X [Meter]')
ylabel('Y [Meter]')
...
title({'Partikelfilter', 'Iteration: 0'});
...
%     part_struct.z(1) = robotPoses(h,2);
    ...
    legend([particle_plot, plots.intsections, plots.robot, plots.nonrays, ...
            plots.rays], {'Partikel', 'Kollisions Punkte', 'Roboter Position', ...
            'Strahlen ohne Kollision', 'Strahlen mit Kollision'});
